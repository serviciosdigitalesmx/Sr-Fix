...
const elLoginScreen = operativoRequireElement('login-screen');
const elApp = operativoRequireElement('app');
const elFechaActual = operativoRequireElement('fecha-actual');
const elPasswordInput = operativoRequireElement('password-input');
const elRememberMe = operativoRequireElement('remember-me');
const elBtnLogin = operativoRequireElement('btn-login');
const elLoginError = operativoRequireElement('login-error');
const elFolioCotizacionInput = operativoRequireElement('folio-cotizacion-input');
const elFolioCotizacionMsg = operativoRequireElement('folio-cotizacion-msg');
const elClienteNombre = operativoRequireElement('cliente-nombre');
const elClienteTelefono = operativoRequireElement('cliente-telefono');
const elClienteEmail = operativoRequireElement('cliente-email');
const elEquipoTipo = operativoRequireElement('equipo-tipo');
const elEquipoModelo = operativoRequireElement('equipo-modelo');
const elEquipoFalla = operativoRequireElement('equipo-falla');
const elFechaPromesa = operativoRequireElement('fecha-promesa');
const elCosto = operativoRequireElement('costo');
const elNotasExtra = operativoRequireElement('notas-extra');
const elChkCargador = operativoRequireElement('chk-cargador');
const elChkPantalla = operativoRequireElement('chk-pantalla');
const elChkPrende = operativoRequireElement('chk-prende');
const elChkRespaldo = operativoRequireElement('chk-respaldo');
const elFotoRecepcion = operativoRequireElement('foto-recepcion');
const elFotoPreviewWrap = operativoRequireElement('foto-preview-wrap');
const elFotoPreview = operativoRequireElement('foto-preview');
const elStep1 = operativoRequireElement('step-1');
const elStep2 = operativoRequireElement('step-2');
const elStep3 = operativoRequireElement('step-3');
const elStep1Ind = operativoRequireElement('step-1-ind');
const elStep2Ind = operativoRequireElement('step-2-ind');
const elStep3Ind = operativoRequireElement('step-3-ind');
const elResCliente = operativoRequireElement('res-cliente');
const elResTelefono = operativoRequireElement('res-telefono');
const elResEmail = operativoRequireElement('res-email');
const elResEquipo = operativoRequireElement('res-equipo');
const elResFalla = operativoRequireElement('res-falla');
const elResChecklist = operativoRequireElement('res-checklist');
const elResFoto = operativoRequireElement('res-foto');
const elResFecha = operativoRequireElement('res-fecha');
const elResCosto = operativoRequireElement('res-costo');
const elBtnGuardar = operativoRequireElement('btn-guardar');
const elExito = operativoRequireElement('exito');
const elFolioGenerado = operativoRequireElement('folio-generado');
...
let fechaTimer = null;
let toastTimer = null;
function operativoRequireElement(id) {
...
function resolverTipoDispositivo(valorSolicitud) {
    ...
    if (v.includes('smart') || v.includes('cel') || v.includes('phone') || v.includes('movil'))
        return 'Smartphone';
    if (v.includes('tablet') || v.includes('ipad'))
        return 'Tablet';
    if (v.includes('lap') || v.includes('notebook') || v.includes('macbook') || v.includes('surface'))
        return 'Laptop';
    if (v.includes('pc') || v.includes('comput') || v.includes('desktop'))
        return 'Computadora';
    return 'Otro';
}
...
function actualizarResumen() {
    ...
    const checks = [];
    if (elChkCargador.checked)
        checks.push('⚡Cargador');
    if (elChkPantalla.checked)
        checks.push('📱Pantalla');
    if (elChkPrende.checked)
        checks.push('🔌Prende');
    if (elChkRespaldo.checked)
        checks.push('💾Respaldo');
    elResChecklist.textContent = checks.join(' • ') || 'Ninguno';
    elResFoto.textContent = fotoRecepcionBase64 ? 'Adjunta' : 'Sin foto';
}
function getOperacionChecks() {
    return {
        cargador: elChkCargador.checked,
        pantalla: elChkPantalla.checked,
        prende: elChkPrende.checked,
...
function generarPDFOrden(tipo = 'previa') {
    ...
    const checksList = [];
    if (datos.checks?.cargador)
        checksList.push('✅ Trae cargador');
    if (datos.checks?.pantalla)
        checksList.push('✅ Pantalla OK');
    if (datos.checks?.prende)
        checksList.push('✅ Equipo prende');
    if (datos.checks?.respaldo)
        checksList.push('✅ Datos respaldados');
