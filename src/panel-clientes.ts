;(function (): void {
  ...
  function badgeEtiqueta(v: unknown): string {
  ...
  function badgeRiesgo(cliente: ClienteRecord): string {
  ...
  function getFiltros(): { texto: string; etiqueta: string; duplicados: string; riesgo: string } {
    return {
  ...
  function setKpis(items: ClienteRecord[], duplicados: string[]): void {
    requireElement<HTMLSpanElement>('kpi-total').textContent = String(items.length);
    requireElement<HTMLSpanElement>('kpi-frecuentes').textContent = String(items.filter((item) => ['frecuente', 'vip'].includes(String(item.ETIQUETA ?? '').toLowerCase())).length);
    requireElement<HTMLSpanElement>('kpi-duplicados').textContent = String(duplicados.length);
    requireElement<HTMLSpanElement>('kpi-morosos').textContent = String(items.filter((item) => item.moroso || String(item.ETIQUETA ?? '').toLowerCase() === 'moroso').length);
  }

  function renderDuplicados(duplicados: string[] = []): void {
  ...
  function refrescarVistaLocal(): void {
    elRows.innerHTML = '';
  ...
  function abrirModal(cliente: ClienteRecord): void {
    requireElement<HTMLInputElement>('cliente-id').value = cliente.ID || '';
    requireElement<HTMLElement>('cliente-title').textContent = `Editar ${cliente.NOMBRE || 'cliente'}`;
    requireElement<HTMLInputElement>('cliente-nombre').value = cliente.NOMBRE || '';
    requireElement<HTMLInputElement>('cliente-telefono').value = String(cliente.TELEFONO || '');
    requireElement<HTMLInputElement>('cliente-email').value = cliente.EMAIL || '';
    requireElement<HTMLSelectElement>('cliente-etiqueta').value = String(cliente.ETIQUETA || '').toLowerCase();
    requireElement<HTMLTextAreaElement>('cliente-notas').value = cliente.NOTAS || '';
  ...
  async function guardarCliente(ev: SubmitEvent): Promise<void> {
    ev.preventDefault();
    const payload = {
      id: requireElement<HTMLInputElement>('cliente-id').value,
      nombre: requireElement<HTMLInputElement>('cliente-nombre').value.trim(),
      telefono: requireElement<HTMLInputElement>('cliente-telefono').value.trim(),
      email: requireElement<HTMLInputElement>('cliente-email').value.trim(),
      etiqueta: requireElement<HTMLSelectElement>('cliente-etiqueta').value,
  ...
  function renderHistorialEquipos(items: ClienteHistorialEquipo[] = []): string {
    if (!items.length) {
  ...
  function renderHistorialCotizaciones(items: ClienteHistorialCotizacion[] = []): string {
    if (!items.length) {
  ...
  function abrirWhatsApp(telefono: unknown): void {
  ...
  async function editarEquipoDesdeHistorial(folio: string): Promise<void> {
  ...
  function crearNuevaOrden(cliente: ClienteRecord): void {
    const draft = {
  ...
  function btnBtnMoreSafe(): void {
    ...
    elBtnMore.addEventListener('click', () => { if (hasMore) void cargarClientes({ append: true }); });

    ['filtro-texto', 'filtro-etiqueta', 'filtro-duplicados', 'filtro-riesgo'].forEach((id) => {
      const el = document.getElementById(id);
      if (!el) return;
      el.addEventListener(id === 'filtro-texto' ? 'input' : 'change', () => {
        if (id === 'filtro-texto') void cargarClientes({ append: false });
    ...
    elRows.addEventListener('click', (event) => {
      ...
      const editEquipo = target?.closest('[data-edit-equipo]') as HTMLElement | null;
    ...
    if (edit) {
      ...
      if (editEquipo) void editarEquipoDesdeHistorial(String(editEquipo.getAttribute('data-edit-equipo') || ''));
    });

    modalCliente.addEventListener('click', (event) => {
      const target = event.target as HTMLElement | null;
      if (target?.id === 'modal-cliente') cerrarModal();
    });

    modalDetalle.addEventListener('click', (event) => {
      const target = event.target as HTMLElement | null;
      if (target?.id === 'modal-detalle') cerrarDetalle();
    });
